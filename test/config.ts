	...
	return new Promise((resolve, reject) => {
		const req = https.request(options, (res: any) => {
			...
			res.on('end', () => {
				...
				const parsedOrEmpty = parsed === undefined || parsed === null ? {} : parsed;
				// If the response was an error, mimic Octokit's behavior by rejecting
				// with an Error that contains a `status` property.
				if (status >= 400) {
					const msg = parsedOrEmpty && parsedOrEmpty.message ? parsedOrEmpty.message : `HTTP ${status}`;
	...
	return {
		rest: {
			repos: {
				compareCommitsWithBasehead: async ({ owner, repo, basehead }: any) =>
					await requestJson('GET', `/repos/${owner}/${repo}/compare/${basehead}`),
				getBranch: async ({ owner, repo, branch }: any) =>
					await requestJson('GET', `/repos/${owner}/${repo}/branches/${branch}`),
		...
		paginate: {
			iterator: async function* (paginatorOpts: any) {
				...
				if (paginatorOpts.direction) qs.set('direction', paginatorOpts.direction);
