...
function mkCfg(extra: any = {}): ConfigLoader {
  ...
  cfg.env = { GITHUB_TOKEN: 't', GITHUB_REF: 'refs/heads/main', GITHUB_REPOSITORY: 'o/r', ...extra } as any;
  return cfg;
}

function mkPull(num = 1): any {
  return { number: num, head: { ref: 'f'+num, label: 'f'+num, repo: { owner: { login: 'o' }, name: 'r' } }, base: { ref: 'main', label: 'main' } };
}
...
describe('AutoUpdater.pulls', () => {
  ...
  it('iterates pages and counts updated PRs', async () => {
    ...
    const iterator = async function* () { yield page1 as any; yield page2 as any; };
    (updater as any).octokit.paginate = { iterator: vi.fn().mockReturnValue(iterator()) };
    (updater as any).octokit.rest = { pulls: { list: { endpoint: { merge: vi.fn().mockReturnValue({}) } } } };
    (updater as any).update = vi.fn().mockResolvedValueOnce(true).mockResolvedValueOnce(false).mockResolvedValueOnce(true);
...
describe('AutoUpdater handlers', () => {
  ...
  it('handleSchedule multiple branches', async () => {
    ...
    const updater = new AutoUpdater(cfg, {} as any);
    (updater as any).pulls = vi.fn().mockResolvedValueOnce(1).mockResolvedValueOnce(2);
    const count = await updater.handleSchedule();
    expect(count).toBe(3);
  });
  it('handleWorkflowRun unsupported event', async () => {
    const ev: any = { workflow_run: { head_branch: 'main', event: 'release' }, repository: { name: 'r', owner: { login: 'o', name: 'o' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
    const res = await updater.handleWorkflowRun();
    expect(res).toBe(0);
  });
  it('handleWorkflowRun missing branch', async () => {
    const ev: any = { workflow_run: { head_branch: '', event: 'push' }, repository: { name: 'r', owner: { login: 'o', name: 'o' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
    const res = await updater.handleWorkflowRun();
    expect(res).toBe(0);
  });
  it('handleWorkflowRun push path', async () => {
    const ev: any = { workflow_run: { head_branch: 'main', event: 'push' }, repository: { name: 'r', owner: { login: 'o', name: 'o' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
  ...
  it('handleWorkflowDispatch', async () => {
    const ev: any = { ref: 'refs/heads/feat', repository: { name: 'r', owner: { login: 'o', name: 'o' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
  ...
  it('handlePush', async () => {
    const ev: any = { ref: 'refs/heads/main', repository: { name: 'r', owner: { login: 'o', name: 'o' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
  ...
  it('handlePullRequest repo null', async () => {
    const ev: any = { action: 'synchronize', pull_request: { head: { repo: null } } };
    const updater = new AutoUpdater(mkCfg(), ev);
    const res = await updater.handlePullRequest();
    expect(res).toBe(false);
  });
  it('handlePullRequest updated path', async () => {
    const ev: any = { action: 'synchronize', pull_request: { number: 9, head: { repo: { owner: { login: 'o' } } }, base: { ref: 'main' } } };
    const updater = new AutoUpdater(mkCfg(), ev);
