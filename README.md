This Action is based on: https://github.com/chinthakagodawita/autoupdate
...
[![Tests](https://github.com/alxndrbauer/autoupdate/actions/workflows/tests.yaml/badge.svg)](https://github.com/alxndrbauer/autoupdate/actions/workflows/tests.yaml)

**autoupdate** is a GitHub Action that auto-updates pull requests branches whenever changes land on their destination branch.

## Usage

Create a file, in your repository, at `.github/workflows/autoupdate.yaml` with the following (JavaScript action usage):
...
jobs:
  autoupdate:
    ...
    steps:
      # (Optional) checkout if other steps need the repo; this action itself only uses the API
      - uses: actions/checkout@v4
      - uses: alxndrbauer/autoupdate@v1
        env:
...
## Migration from Docker-based version

Earlier versions required `docker://chinthakagodawita/autoupdate-action:v1`. The action now ships as a compiled JavaScript bundle and runs with `using: node20`, reducing cold start time and removing the need to build/pull an image. To migrate just replace any `uses: docker://...autoupdate-action:v1` lines with `uses: alxndrbauer/autoupdate@v1`. All environment variables and behaviour remain the same.
